...
- **Fragespiel**: Start screen picks group (Friends/Couple), language (DE/EN), round length (16/32/64, default 32), sort (By intensity / Random, By intensity default), and categories. Category filter: multi-select chips w/ per-category counts over the whole active pool; state is the _deselected_ set (empty = all on, so new DB categories default on) and is in-memory only. `categories.ts` owns `poolFor(questions, group, deselected)` — the single group+category filter used by both `pickGroup` and `reshuffle`. Group badges show the filtered count and the button disables at 0 (the `deck.length === 0` render gate would otherwise be a dead click). `questions.category` is nullable free text in English; nulls bucket under `UNCATEGORIZED` and `categoryLabel()` in `i18n.ts` localizes known values for both chips and card kicker, falling back to the raw string. `buildRound()` balances the deck across intensity buckets 1-3 (round-robin draw), random category order; `displayDeck` re-sorts by difficulty when "By intensity" is chosen. Non-wrapping play deck: depth = `p - index`, so the stack shrinks toward the end; final cards swipe away one by one, then a centered "Play another round" CTA returns to the start screen. Swipe via `useSwipe.ts` (PointerEvents, 100px threshold); cards exit + re-enter on the left along one shared trajectory (`pathTransform`, p=0 TOP → p=1 FLY) so release continues the drag. prev disabled at index 0, next at index L. Footer hidden via route gate in `Shell.tsx`.
