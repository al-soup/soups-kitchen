...
- **KB sort + grouping**: `SortControl` (`_form/SortControl.tsx`) offers Newest / Oldest / By topic; URL param `sort` (default `newest` omitted from URL, `parseSort` in `_form/filterParams.ts`). Sorting lives in the RPC (`p_sort` arg, migration `20260726000000_search_knowledge_sort.sql`) because the list is server-paginated. Relevance ranking applies only in `newest`. RPC also returns `topic_name` (primary topic = alphabetically first `type='topic'` tag, NULLs last) + `group_count` (window count over the filtered set); in `topic` mode the page emits a full-width group header row whenever `topicName` changes from the previous entry — correct across page boundaries since ordering is global.
- **KB card reveal**: Summary is hidden until revealed and scrolls inside a fixed `--kb-reveal-height` box (no line clamp). Hover **and `:focus-within`** reveal are both gated behind `@media (hover: hover)`; on coarse pointers a stretched `.tapToggle` button toggles `data-expanded` on the card and only the "Read" link navigates. Keep the focus rules hover-scoped: `.tapToggle` retains focus after a tap, so a focus-driven reveal pins the card open and the collapse tap looks dead. Whole-card click on desktop comes from a stretched `.cardOverlay` link (`aria-hidden`, not focusable, hidden on touch) — `.reveal` sits above it and `.tapToggle` (`z-index: 3`) so summary text stays scrollable/selectable, which also means the summary area is deliberately **not** a toggle target (tap elsewhere on the card to collapse; e2e taps `{x: 40, y: 24}` for this reason).
