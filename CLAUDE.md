...
- **KB card reveal**: Summary is hidden until revealed and scrolls inside a fixed `--kb-reveal-height` box (no line clamp). Hover reveal is gated behind `@media (hover: hover)`; on coarse pointers a stretched `.tapToggle` button toggles `data-expanded` on the card and only the "Read" link navigates. Whole-card click on desktop comes from `.readCta::after` stretched inside the hover media query — `.reveal` sits above it (`z-index: 2`) so summary text stays scrollable/selectable.
